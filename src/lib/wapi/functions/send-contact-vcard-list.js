...
export async function sendContactVcardList(to, contacts){
        ...
        if(typeof to != "string"){
            var text = "incorrect parameter, insert an string. Example: '222222222222@c.us'";
            return WAPI.scope(to,true,null, text);
        }
        if (!Array.isArray(contacts)){
            var text = "incorrect parameter, insert an array. Example: ['222222222222@c.us', '333333333333@c.us, ... ]";
            return WAPI.scope(to,true,null, text);
        }
        if(contacts.length === 1){
            var text = "Enter more than one number to send. Example: ['222222222222@c.us', '333333333333@c.us, ... ]";
            return WAPI.scope(to,true,null, text);
        }
        var ad = [to].concat(contacts), chat = await WAPI.sendExist(ad);
        if(chat.erro === false || chat.__x_id){
        var ListChat = await Store.Chat.get(to),
             tempMsg = Object.create(Store.Msg.models.filter(msg => msg.__x_isSentByMe && !msg.quotedMsg)[0]),
