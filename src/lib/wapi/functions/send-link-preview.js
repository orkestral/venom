...
export async function sendLinkPreview(chatId, url, text){
    ...
    const _Path = {
        ...
        "End": "(\\#[-a-z\\d_]*)?$",
        Reg : () => {
            return new RegExp(_Path.Protocol+_Path.Domain+_Path.IP+_Path.Port+_Path.Query+_Path.End,'i');
        }
    };
    if(!_Path.Reg().test(url)){
            var text = "Use a valid HTTP protocol. Example: https://www.youtube.com/watch?v=V1bFr2SWP1";
    ...
    if(chat.erro === false || chat.__x_id){
        ...
        var result = Promise.all((ListChat)? await chat.sendMessage(text.includes(url)? text : `${url}\n${text}`, {linkPreview}) : "");
