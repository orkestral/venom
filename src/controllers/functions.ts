...
export async function useHere(
  wapage: Page,
  Session: string,
  statusUseHere: (statusGet: string, session: string) => void
) {
...
export function scrapQrcodeTime(
  ...
  statusF: (statusGet: string, session: string) => void,
  catQR: (qrCode: string, asciiQR: string, attempt: number) => void
): NodeJS.Timeout {
  ...
  intervalScrapeQrcode = setInterval(async () => {
    ...
    if (browser['isClose'] != undefined || browser.isConnected() === false) {
      if (statusF) {
    ...
    if (inject === true) {
      ...
      await webpackJsonpWI(waPage, async (state: string) => {
        ...
        if (state === 'CONFLICT' && mergedOptions.usehere === true) {
          await useHere(waPage, Session, (statusGet, session) => {
            statusWS(statusGet, Session);
          });
        }
